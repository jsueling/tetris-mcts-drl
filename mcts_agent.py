...
class MCTSAgent:
    ...
    def train_ensemble(self, episodes=10000, n_workers=10):
        ...
        to learn to play Tetris. Each step in an episode consists of running parallel
        ...
        # Attempt to start from previous checkpoint if it exists
        results = self.checkpoint.load()

        episode_scores = results.get("episode_scores", [])
        rolling_avg_scores = results.get("rolling_avg_scores", [])
        steps_per_episode = results.get("steps_per_episode", [])
        episode_times = results.get("episode_times", [])
        prev_step_count = step_count = results.get("total_steps", 0)
        starting_episode = results.get("total_episodes", 0)
        ...
        try:
            for episode in tqdm(
                range(starting_episode, episodes),
                ...
                rolling_avg_scores.append(round(float(np.mean(episode_scores[-100:])), 2))
                steps_per_episode.append(step_count - prev_step_count)
                end_time = time.time()
                episode_times.append(round(float(end_time - start_time), 2))
                ...
                # Light save, so that results can be viewed immediately
                self.checkpoint.save(results=results)
                ...
                if episode > 0 and episode % 50 == 0:
                    # Hard save is computationally expensive, so it is done
                    # less frequently but allows complete state restoration
            ...
            # Cleanup the inference server, freeing resources
            inference_server.stop()

    async def train_async(self, episodes=10000, n_workers=10):
        """
        Run the training loop for an MCTS agent that learns to play Tetris.
        This agent models the tree using chance nodes and updates a shared tree with
        ...
        response_queues = { worker_id: asyncio.Queue() for worker_id in range(n_workers) }
        request_queue = asyncio.Queue()

        # Create and start the inference server which handles model inference
        # requests sent by workers.
            ...
            for episode in tqdm(
                ...
                while not done:
                    ...
                    await run_async_mcts(
                        ...
                        n_workers=n_workers,
                        is_episode_start=(step_count == prev_step_count)
                    )
